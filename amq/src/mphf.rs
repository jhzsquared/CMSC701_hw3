...
pub fn query_mphf(keys_new: &(Vec<String>, Vec<String>), phf: &Mphf<String>) -> (i32, i32){
...
pub fn query_fpt(keys_new: &(Vec<String>, Vec<String>), fingerprint: &Vec<i32>, phf: &Mphf<String>, hash_builder: &RandomState, b: &usize,) -> (i32, i32){
    let mut fpr = 0;
    for s in keys_new.0.iter(){
        let mpfh_tmp =  phf.try_hash(s);
        if mpfh_tmp!=None{
            // check if it matches the fingerprint array, then definitely false pos
            if fingerprint[mpfh_tmp.unwrap() as usize]==get_bbits(&hash_builder, s, &b) as i32 {
                fpr+=1;
            }   
        }
    }
    ...
    for s in keys_new.1.iter(){
        ...
        if mpfh_tmp==None{
            fng+=1;
        } else if fingerprint[mpfh_tmp.unwrap() as usize] != get_bbits(&hash_builder, s, &b) {
            fng+=1;
        }
    }
    return (fpr, fng)
